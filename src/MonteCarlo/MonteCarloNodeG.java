...
public class MonteCarloNodeG {
	...
	public int computeBestUTC() {
		...
		ArrayList<Integer> bestDirection = new ArrayList<>();
		for (int i=0;i<modelPossibility;i++) {
			...
			double ni = 1;
			if (getChild(i) != null) {
				wi = getChild(i).pointsEarned;
				ni = getChild(i).nodeTry;
			}
